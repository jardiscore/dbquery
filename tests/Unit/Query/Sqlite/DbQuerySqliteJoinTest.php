 ...
 */
class DbQuerySqliteJoinTest extends TestCase
{
    public function testInnerJoinWithConstraint(): void
    {
        ...
        $sql = $query->select('*')->from('users')->sql('sqlite', false);
    ...
    {
        $query = new DbQuery();
        $sql = $query->select('*')->from('users')->crossJoin('posts')->sql('sqlite', false);

...
phpunit-unit: ## Run unit tests only
	$(DOCKER_COMPOSE) run --rm phpcli vendor/bin/phpunit --bootstrap ./tests/bootstrap.php --testsuite "Unit Tests"

phpunit-integration: ## Run integration tests only (requires: make start)
	$(DOCKER_COMPOSE) run --rm phpcli vendor/bin/phpunit --bootstrap ./tests/bootstrap.php --testsuite "Integration Tests"
.PHONY: phpunit
